...
static void Get_KEY_val(FIL* file,char*KEY_section,char*KEY_secval,char getbuff[])
{
	...
	f_lseek(&gfile, 0x0);
	while(f_gets((TCHAR*)buf, MAX_KEY_LEN, &gfile) != NULL)
	{
		Trim((char*)buf);
    ...
    if (buf[0] != '#' && (buf[0] != '/' || buf[1] != '/'))
    {
			...
			else if (strstr((const char*)buf, "*/") != NULL)
			{
				text_comment = 0;
				continue;
			}
...
static u32 Get_CHT_val(FIL* file,char*KEY_section,char*KEY_secval/*,char getbuff[]*/)
{
	...
	f_lseek(&gfile, 0x0);
	while(f_gets((TCHAR*)buf, MAX_BUF_LEN, &gfile) != NULL)
	{
		Trim((char*)buf);
    // to skip text comment with flags /* ...*/
    if (buf[0] != '#' && (buf[0] != '/' || buf[1] != '/'))
...
//------------------------------------------------------------------
static unsigned long str2hex(const unsigned char*str)
{
	unsigned long sum=0;
	unsigned long i;
